...
class BearCallSpreadStrategy(BaseStrategy):
    """
    Bear Call Spread Strategy (Conservative)
    
    Bearish strategy with limited risk and limited profit.
    Structure: Sell lower strike call (collect premium), Buy higher strike call (protection)
    
    Market View: Mildly to moderately bearish
    Max Profit: Net premium collected
    Max Loss: (Strike difference - Net premium)
    """
    
    def __init__(self):
        super().__init__("Bear Call Spread", StrategyType.CONSERVATIVE)
        ...
        self.target_dte = 15  # Prefer weekly options
        ...
        self.min_moneyness = 0.02  # Minimum 2% OTM for short call
        
    async def generate_signals(self, market_data: MarketData) -> List[StrategySignal]:
        if not self._is_market_suitable_for_bear_call_spread(market_data):
            return []
        ...
        try:
            if "NIFTY" in market_data.symbol or "BANKNIFTY" in market_data.symbol:
                signal = await self._create_bear_call_spread_signal(market_data)
                if signal:
                    signals.append(signal)
                    
        except Exception as e:
            print(f"Error generating Bear Call Spread signals: {e}")
            
        return signals
    
    async def _create_bear_call_spread_signal(self, market_data: MarketData) -> Optional[StrategySignal]:
        ...
        # Calculate strikes for Bear Call Spread
        # We want to sell a call above current price (collect premium)
        # And buy a call further above (protection)
        
        # Short call strike (lower strike) - should be OTM
        atm_strike = round(spot_price / strike_interval) * strike_interval
        min_otm_distance = max(2 * strike_interval, spot_price * self.min_moneyness)
        short_call_strike = atm_strike + min_otm_distance
        
        # Long call strike (higher strike) - protection
        long_call_strike = short_call_strike + self.spread_width
        
        # Create the two legs of Bear Call Spread
        legs = [
            ...
            # Buy higher strike call (pay premium for protection)
            OptionLeg(
        ...
        breakeven_point = short_call_strike + net_premium
        
        # Calculate probability of profit (simplified)
        # Profit if spot stays below breakeven
        distance_to_breakeven = breakeven_point - spot_price
    ...
    def _is_market_suitable_for_bear_call_spread(self, market_data: MarketData) -> bool:
        ...
        # Bear Call Spread works when we expect price to stay below short strike
        # Better in downtrending or sideways markets
        return True
    ...
    def validate_signal(self, signal: StrategySignal, current_positions: int) -> bool:
        ...
        # Validate Bear Call Spread structure (2 legs, both calls)
    ...
    def calculate_position_size(self, signal: StrategySignal, available_capital: float) -> int:
        # Conservative position sizing for Bear Call Spread
