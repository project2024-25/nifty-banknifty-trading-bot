...
class BullCallSpreadStrategy(BaseStrategy):
    """
    Bull Call Spread Strategy (Conservative)
    ...
    def __init__(self):
        super().__init__("Bull Call Spread", StrategyType.CONSERVATIVE)
        ...
        self.max_moneyness = 0.03  # Maximum 3% ITM for long call
        
    async def generate_signals(self, market_data: MarketData) -> List[StrategySignal]:
        if not self._is_market_suitable_for_bull_call_spread(market_data):
            return []
        ...
        try:
            if "NIFTY" in market_data.symbol or "BANKNIFTY" in market_data.symbol:
                signal = await self._create_bull_call_spread_signal(market_data)
                if signal:
                    signals.append(signal)
                    
        except Exception as e:
            print(f"Error generating Bull Call Spread signals: {e}")
            
        return signals
    
    async def _create_bull_call_spread_signal(self, market_data: MarketData) -> Optional[StrategySignal]:
        ...
        # Calculate strikes for Bull Call Spread
        # Buy lower strike call (closer to ATM for directional benefit)
        # Sell higher strike call (to reduce cost)
        
        atm_strike = round(spot_price / strike_interval) * strike_interval
        
        # Long call strike (can be slightly ITM or ATM)
        max_itm_distance = spot_price * self.max_moneyness
        long_call_strike = max(atm_strike - max_itm_distance, atm_strike)
        long_call_strike = round(long_call_strike / strike_interval) * strike_interval
        
        # Short call strike (higher strike to cap profits)
        short_call_strike = long_call_strike + self.spread_width
        
        # Create the two legs of Bull Call Spread
        legs = [
            # Buy lower strike call (pay premium)
            OptionLeg(
            ...
            # Sell higher strike call (collect premium to reduce cost)
            OptionLeg(
        ...
        breakeven_point = long_call_strike + net_premium
        
        # Calculate probability of profit (simplified)
        # Profit if spot goes above breakeven
        distance_from_spot = breakeven_point - spot_price
    ...
    def _is_market_suitable_for_bull_call_spread(self, market_data: MarketData) -> bool:
        ...
        # Bull Call Spread works when we expect moderate upward movement
        return True
    ...
    def _estimate_premium(self, strike: float, spot: float, option_type: OptionType, iv: float) -> float:
        ...
        if points_from_atm <= 100:  # ATM/Near ATM
            time_value = base_time_value * 100  # Higher for ATM calls
    ...
    def validate_signal(self, signal: StrategySignal, current_positions: int) -> bool:
        ...
        # Validate Bull Call Spread structure (2 legs, both calls)
        ...
        # Check that long call has lower strike than short call
        long_call = buys[0]
        short_call = sells[0]
